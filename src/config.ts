...
function parseProjects(value: unknown): DevcardProject[] | undefined {
  ...
  return value.map((entry, index) => {
    ...
    return {
      ...
      status: status as DevcardProject['status'] | undefined,
    };
  });
}

function parseLinkCollection(value: unknown, label: string): DevcardLink[] | undefined {
  if (value == null) {
    return undefined;
  }
  if (!Array.isArray(value)) {
...
function parseWritingCollection(value: unknown, label: string): DevcardWriting[] | undefined {
  if (value == null) {
    return undefined;
  }
  if (!Array.isArray(value)) {
...
function withOptional<T extends object, K extends string, V>(target: T, key: K, value: V | undefined): T & Partial<Record<K, V>> {
  if (value === undefined) {
    return target;
  }
...
function parseProfile(value: unknown): DevcardProfile {
